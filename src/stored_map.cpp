                                                                ...
                                                                y_headers)
{
    ...
    if ((x_size * y_size) == map_element_count)
    {
        ...
        if (nullptr != dest)
        {
            bool read_data_successful = EEPROM::read_nvs_map_data(eeprom_key_name, dest, default_map, map_element_count);
            if (read_data_successful)
            {
                bool data_added = this->add_data(dest, map_element_count);
                if (data_added)
                {
                    // Everything OK!
