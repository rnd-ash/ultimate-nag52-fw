...
typedef union {
    ...
    // Sets actual gear
    void set_GS_218H_GIC(uint8_t value){ raw = (raw & 0xfffff0ffffffffff) | ((uint64_t)value & 0xf) << 40; }
    // Gets actual gear
    GS_218H_GIC get_GIC() { return (GS_218H_GIC)(raw >> 40 & 0xf); }

    // Sets target gear
    void set_GS_218H_GZC(uint8_t value){ raw = (raw & 0xffff0fffffffffff) | ((uint64_t)value & 0xf) << 44; }
    // Gets target gear
    ...
    void set_GS_218H_FPC_AAD(uint8_t value){ raw = (raw & 0xfffffffffcffffff) | ((uint64_t)value & 0x3) << 24; }
    ...
    void set_GS_218H_FEHLPRF_ST(uint8_t value){ raw = (raw & 0xffffffffffffff3f) | ((uint64_t)value & 0x3) << 6; }
