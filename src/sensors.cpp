...
bool Sensors::init_sensors(){
    ...
    if (check_esp_func(gpio_set_direction(pcb_gpio_matrix->vsense_pin, GPIO_MODE_INPUT), "Failed to set PIN_VBATT to Input!") &&
        check_esp_func(gpio_set_direction(pcb_gpio_matrix->atf_pin, GPIO_MODE_INPUT), "Failed to set PIN_ATF to Input!") &&
        check_esp_func(gpio_set_direction(pcb_gpio_matrix->n2_pin, GPIO_MODE_INPUT), "Failed to set PIN_N2 to Input!") &&
        check_esp_func(gpio_set_direction(pcb_gpio_matrix->n3_pin, GPIO_MODE_INPUT), "Failed to set PIN_N3 to Input!"))
    {
        // Set RPM pins to pullup
        if (check_esp_func(gpio_set_pull_mode(pcb_gpio_matrix->n2_pin, GPIO_PULLUP_ONLY), "Failed to set PIN_N2 to Input!") &&
            check_esp_func(gpio_set_pull_mode(pcb_gpio_matrix->n3_pin, GPIO_PULLUP_ONLY), "Failed to set PIN_N3 to Input!"))
        {
            // Configure ADC2 for analog readings
            if (check_esp_func(adc2_config_channel_atten(pcb_gpio_matrix->sensor_data.atf_channel, ADC_ATTEN_11db), "Failed to set ADC attenuation for PIN_ATF!") &&
                check_esp_func(adc2_config_channel_atten(pcb_gpio_matrix->sensor_data.batt_channel, ADC_ATTEN_11db), "Failed to set ADC attenuation for PIN_VBATT!"))
            {
                // Characterise ADC2
                ...
                if (check_esp_func(pcnt_unit_config(&pcnt_cfg_n2), "Failed to configure PCNT for N2 RPM reading!") &&
                    check_esp_func(pcnt_unit_config(&pcnt_cfg_n3), "Failed to configure PCNT for N3 RPM reading!"))
                {
                    // Pause PCNTs unit configuration is complete
                    if (check_esp_func(pcnt_counter_pause(PCNT_N2_RPM), "Failed to pause PCNT N2 RPM!") &&
                        check_esp_func(pcnt_counter_pause(PCNT_N3_RPM), "Failed to pause PCNT N3 RPM!"))
                    {
                        // Clear their stored values (If present)
                        if (check_esp_func(pcnt_counter_clear(PCNT_N2_RPM), "Failed to clear PCNT N2 RPM!") &&
                            check_esp_func(pcnt_counter_clear(PCNT_N3_RPM), "Failed to clear PCNT N3 RPM!"))
                        {
                            ...
                            if (check_esp_func(pcnt_set_filter_value(PCNT_N2_RPM, 1000), "Failed to set filter for PCNT N2!") &&
                                check_esp_func(pcnt_set_filter_value(PCNT_N3_RPM, 1000), "Failed to set filter for PCNT N3!") &&
                                check_esp_func(pcnt_filter_enable(PCNT_N2_RPM), "Failed to enable filter for PCNT N2!") &&
                                check_esp_func(pcnt_filter_enable(PCNT_N3_RPM), "Failed to enable filter for PCNT N3!"))
                            {
                                // Now install and register ISR interrupts
                                if (check_esp_func(pcnt_isr_service_install(0), "Failed to install ISR service for PCNT!") &&
                                    check_esp_func(pcnt_isr_handler_add(PCNT_N2_RPM, &on_pcnt_overflow_n2, nullptr), "Failed to add PCNT N2 to ISR handler!") &&
                                    check_esp_func(pcnt_isr_handler_add(PCNT_N3_RPM, &on_pcnt_overflow_n3, nullptr), "Failed to add PCNT N3 to ISR handler!"))
                                {
                                    // Enable interrupts on hitting hlim on PCNTs
                                    if (check_esp_func(pcnt_event_enable(PCNT_N2_RPM, pcnt_evt_type_t::PCNT_EVT_H_LIM), "Failed to register event for PCNT N2!") &&
                                        check_esp_func(pcnt_event_enable(PCNT_N3_RPM, pcnt_evt_type_t::PCNT_EVT_H_LIM), "Failed to register event for PCNT N3!"))
                                    {
                                        // Resume counting
                                        if (check_esp_func(pcnt_counter_resume(PCNT_N2_RPM), "Failed to resume PCNT N2 RPM!") &&
                                            check_esp_func(pcnt_counter_resume(PCNT_N3_RPM), "Failed to resume PCNT N3 RPM!"))
                                        {
