...
enum class EZS_240H_FZGVERSN {
    START = 0, /** Status when the respective series */
    V1 = 1, /** BR 220: AJ 99 */
...
typedef union {
    ...
    void set_EZS_240H_LL_RLC(uint8_t value){ raw = (raw & 0xffcfffffffffffff) | ((uint64_t)value & 0x3) << 52; }
    ...
    void set_EZS_240H_ESP_BET(uint8_t value){ raw = (raw & 0xffffffff9fffffff) | ((uint64_t)value & 0x3) << 29; }
    ...
    void set_EZS_240H_ART_ABW_BET(uint8_t value){ raw = (raw & 0xfffffffffff3ffff) | ((uint64_t)value & 0x3) << 18; }
    ...
    void set_EZS_240H_ST3_BET(uint8_t value){ raw = (raw & 0xffffffffffcfffff) | ((uint64_t)value & 0x3) << 20; }
    ...
    void set_EZS_240H_ST2_BET(uint8_t value){ raw = (raw & 0xffffffffff3fffff) | ((uint64_t)value & 0x3) << 22; }
    ...
    // Sets country code
    void set_EZS_240H_LDC(uint8_t value){ raw = (raw & 0xfffffffffffffffc) | ((uint64_t)value & 0x3) << 0; }
    // Gets country code
    EZS_240H_LDC get_LDC() { return (EZS_240H_LDC)(raw >> 0 & 0x3); }

    // Sets model-dependent vehicle version (only 220/215/230)
    void set_EZS_240H_FZGVERSN(uint8_t value){ raw = (raw & 0xffffffffffffffe3) | ((uint64_t)value & 0x7) << 2; }
